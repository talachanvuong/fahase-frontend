...
export default function CategoryList() {
  ...
  useEffect(() => {
    api.get("/category/getAll").then(res => setCategories(res.data)).catch(err => console.error(err));
  }, []);

  const handleAdd = () => {
    const name = prompt("Tên loại mới:");
    if (!name) return;
    api.post("/category/add", { name }).then(res => setCategories(prev => [...prev, res.data]));
  };

  const handleDelete = (id) => {
    if (!confirm("Xóa loại sản phẩm này?")) return;
    api.delete(`/category/remove/${id}`).then(() => setCategories(prev => prev.filter(c => c._id !== id)));
  };
